...
sgraph1 <-
  ...
  geom_density(aes(x = bullyingRate, fill = recogBullying, alpha = 0.3)) +
...
sgraph2 +
  geom_boxplot(lwd = 0.2, color = "black") +
  geom_jitter(alpha = .1) +
...
```{r}
dummyPred <- c(c(F, F, F), c(F, F, T), c(F, T, F), c(F, T, T),
               c(T, F, F), c(T, F, T), c(T, T, F), c(T, T, T),)
```
...
### Is funding really the biggest issue?
...
schooldata %>%  
  ggplot() +
  aes(x = limitFunding, fill = as.character(bullyingRate)) +
